...
Blockly.Blocks.initializes_loop={
	...
	onchange: function (event) {
	  ...
	  document.getElementById("terminal-body").innerHTML = code
		.replace(/</g, "&lt;")
		.replace(/>/g, "&gt;")
		.replace(/\n/g, "<br>")
		.replace(/ /g, "&nbsp;");
	  ...
	  var enabledBlockList = ["initializes_loop", "initializes_restart"];
	  var variableBlockList = [
		"variables_set",
		"variables_set1",
		"variables_set7",
	  ];
	  ...
	  for (var i = 0; i < blocks.length; i++) {
		...
		console.log(p);
		if (
		  enabledBlockList.includes(p.type) ||
		  variableBlockList.includes(p.type) ||
		  (p.previousConnection == null && p.outputConnection == null)
		) {
		  if (topCheck) blocks[i].setEnabled(true);
		  if (
			variableGlobalBlockList.includes(blocks[i].type) &&
			blocks[i].getField("POSITION")
		  ) {
			if (blocks[i].getFieldValue("POSITION") == "global") continue;
		  } else continue;
		}
		p =
		  p.getParent() || p.getPreviousBlock()
			? p.getParent() || p.getPreviousBlock()
			: "";
		while (p) {
		  if (
		...
		if (blocks[i].getParent() && blocks[i].getPreviousBlock()) {
		  if (
			variableBlockList.includes(p.type) &&
			variableBlockList.includes(blocks[i].getParent().type)
		  ) {
